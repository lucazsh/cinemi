...
async function nextQuestion() {
    ...
    if (current === 0) {
        ...
        if (selected) {
            answer = selected.value || selected.id;
        }
    }
    ...
    if (current < total) {
        ...
        const quizAnswers = {
            ...
            movieGenre: questions[10].querySelector('input:checked')?.value,
            streamingPlatform: questions[11].querySelector('input:checked')?.value
        };
    ...
    inputAge.addEventListener('input', function(e) {
        ...
        if (val === '67' && !active) {
            ...
            wrap.innerHTML = `
                ...
                <div class="hand-row">
                    <div class="h-anim h-l">🫴</div>
                    <div class="h-anim h-r">🫴</div>
                </div>
            `;
            ...
            setTimeout(() => {
                ...
                wrap.querySelectorAll('.s-67, .big-67, .h-anim').forEach(el => {
                ...
                setTimeout(() => {
                    wrap.remove();
                    document.body.classList.remove('shake-eff');
                    active = false;
                }, 500);
            }, 4500); 
        }
...
css.textContent = `
    ...
    .zoom-out {
        animation: zoom-out-anim 0.5s ease-in forwards !important;
    }
    ...
    .layer-67 {
        ...
        animation: entry-zoom 0.4s ease-out forwards;
    }
    ...
    .big-67 {
        ...
        text-shadow: 8px 8px 0px #000 !important;
        animation: rotate-move 0.3s infinite alternate ease-in-out;
    }
    ...
    .h-anim { font-size: 35vw !important; }
    
    .h-l { animation: alt-y 0.5s infinite alternate ease-in-out; }
    .h-r { 
        transform: scaleX(-1); 
        animation: alt-y-rev 0.5s infinite alternate ease-in-out; 
    }
    ...
    @keyframes alt-y-rev {
        from { transform: scaleX(-1) translateY(-60px); }
        to { transform: scaleX(-1) translateY(0); }
    }

    .shake-eff {
        animation: intense-shake 0.08s infinite !important;
    }
    ...
    @media (min-width: 768px) {
        .big-67 { font-size: 400px !important; }
        .s-67 { font-size: 40px !important; }
        .h-anim { font-size: 180px !important; }
    }
`;
