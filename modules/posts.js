    ...
    if (files && files.length) {
        ...
        if (imgs.length) {
            ...
            imgs.forEach(f => {
                ...
                filesHtml += `<img src="${src}" onclick="openImgViewer(this)" style="height:220px;width:auto;object-fit:cover;flex-shrink:0;border-radius:20px;display:block;cursor:pointer;"/>`;
            });
            filesHtml += '</div>';
        }
        if (others.length) {
    ...
    wrapper.innerHTML = `
        <div class="pt" data-username="${escapeHtml(username)}">
            ...
            <div style="display:block;min-width:0;flex:1;">
                <span class="usr usr-post" data-username="${escapeHtml(username)}">${escapeHtml(username)} <span class="compat-badge" data-for="${escapeHtml(username)}"></span></span>
...
function _showIvImage(imgEl, animate) {
    ...
    if (_ivSourcePost) {
        ...
        if (replyBtn) {
            const rb = replyBtn.cloneNode(true);
...
function syncIvActions() {
    ...
    if (srcLike && ivLike) {
        ...
        if (isLiked !== wasLiked) {
            const ivHeart = ivLike.querySelector(isLiked ? '.heart-fill' : '.heart-out');
            const srcHeart = srcLike.querySelector(isLiked ? '.heart-fill' : '.heart-out');
            if (ivHeart && srcHeart) {
                const cls = Array.from(srcHeart.classList).find(c => c === 'heart-pop' || c === 'heart-unpop');
                if (cls) {
                    ivHeart.classList.remove('heart-pop', 'heart-unpop');
                    void ivHeart.offsetWidth;
                    ivHeart.classList.add(cls);
                    setTimeout(() => ivHeart.classList.remove(cls), 300);
                }
            }
        }
    }
    ...
    if (srcReply && ivReply) {
        ...
        if (srcCount && ivCount) ivCount.textContent = srcCount.textContent;
        const srcWrap = srcReply.querySelector('.reply-count-wrap');
        const ivWrap = ivReply.querySelector('.reply-count-wrap');
        if (srcWrap && ivWrap) ivWrap.style.cssText = srcWrap.style.cssText;
    }
}
...
function _ivNav(dir) {
    ...
    _ivIndex = next;
    const viewerImg = document.getElementById('img-viewer-img');
    viewerImg.style.transition = 'transform 0.3s cubic-bezier(0.4,0,0.2,1)';
    viewerImg.style.transform = `translateX(${dir < 0 ? '100%' : '-100%'})`;
    setTimeout(() => {
...
function _initIvSwipe() {
    ...
    const top = document.getElementById('img-viewer-top');
    const actions = document.getElementById('img-viewer-actions');
    let startX = 0, startY = 0, isDragging = false, mode = null;

    const nextImg = document.createElement('img');
    nextImg.style.cssText = 'position:absolute;width:100%;height:auto;max-height:75vh;object-fit:contain;pointer-events:none;opacity:0;';
    viewer.appendChild(nextImg);
    ...
    viewerImg.onpointermove = (e) => {
        ...
        if (!mode) {
            if (Math.abs(dy) > 8 || Math.abs(dx) > 8)
                mode = Math.abs(dy) > Math.abs(dx) ? 'y' : 'x';
        }

        if (mode === 'y' && dy > 0) {
            const prog = Math.min(dy / 300, 1);
            viewerImg.style.transform = `translateY(${dy}px) scale(${1 - prog * 0.15})`;
            viewer.style.background = `rgba(0,0,0,${1 - prog * 0.9})`;
            top.style.opacity = `${Math.max(0, 1 - prog * 5)}`;
            actions.style.opacity = `${Math.max(0, 1 - prog * 5)}`;
            nextImg.style.opacity = '0';
        }

        if (mode === 'x') {
            const dir = dx < 0 ? 1 : -1;
            const neighborIndex = _ivIndex + (dx < 0 ? 1 : -1);
            viewerImg.style.transform = `translateX(${dx}px)`;

            if (neighborIndex >= 0 && neighborIndex < _ivAllImgs.length) {
                nextImg.src = _ivAllImgs[neighborIndex].src;
                const offset = dx < 0 ? window.innerWidth : -window.innerWidth;
                nextImg.style.transform = `translateX(${offset + dx}px)`;
    ...
    viewerImg.onpointerup = (e) => {
        ...
        if (mode === 'y' && dy > 90) {
            ...
            viewerImg.style.transition = 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1)';
            viewerImg.style.transform = 'translateY(0) scale(1)';
            viewer.style.background = '#000';
            top.style.opacity = '';
            actions.style.opacity = '';
        } else if (mode === 'x' && Math.abs(dx) > 60) {
            const dir = dx < 0 ? 1 : -1;
            const neighborIndex = _ivIndex + dir;
            if (neighborIndex >= 0 && neighborIndex < _ivAllImgs.length) {
                viewerImg.style.transition = 'transform 0.3s cubic-bezier(0.4,0,0.2,1)';
                nextImg.style.transition = 'transform 0.3s cubic-bezier(0.4,0,0.2,1)';
                viewerImg.style.transform = `translateX(${dx < 0 ? '-100%' : '100%'})`;
                nextImg.style.transform = 'translateX(0)';
                setTimeout(() => {
                    _ivIndex = neighborIndex;
                    nextImg.style.opacity = '0';
                    nextImg.style.transition = 'none';
                    _showIvImage(_ivAllImgs[_ivIndex], false);
                    viewerImg.style.transition = 'none';
            ...
            viewerImg.style.transition = 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1)';
            viewerImg.style.transform = 'translateX(0)';
            nextImg.style.transition = 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1)';
            const offset = dx < 0 ? window.innerWidth : -window.innerWidth;
            nextImg.style.transform = `translateX(${offset}px)`;
            setTimeout(() => { nextImg.style.opacity = '0'; }, 350);
        }
    };
}
