...
function showSkeletonLoaders() {
    ...
    if (moodContainer) {
        ...
        Array(3).fill(0).forEach(() => {
            ...
            div.style.cssText = 'width:40%;aspect-ratio:2/3;border-radius:15px;background:linear-gradient(90deg,var(--bg-primary) 25%,var(--bg-secondary) 50%,var(--bg-primary) 75%);background-size:200% 100%;animation:shimmer 1.5s infinite;flex-shrink:0;';
            if (seeMore) moodContainer.insertBefore(div, seeMore);
            else moodContainer.appendChild(div);
        });
    }
    const trendingContainer = document.getElementById('trending');
    if (trendingContainer) {
        const seeMore = trendingContainer.querySelector('.mov-see-more');
        trendingContainer.querySelectorAll('.tr-poster-wrap').forEach(el => el.remove());
        Array(3).fill(0).forEach(() => {
            const div = document.createElement('div');
            div.className = 'tr-poster-wrap skeleton-loader';
            div.style.cssText = 'background:linear-gradient(90deg,var(--bg-primary) 25%,var(--bg-secondary) 50%,var(--bg-primary) 75%);background-size:200% 100%;animation:shimmer 1.5s infinite;';
...
async function loadTrendingMovies() {
    try {
        ...
        if (trendingContainer && data.results) {
            const seeMore = trendingContainer.querySelector('.mov-see-more');
            trendingContainer.querySelectorAll('.tr-poster-wrap, .skeleton-loader').forEach(el => el.remove());
            data.results.slice(0, 6).forEach((movie, idx) => {
                const posterUrl = movie.poster_path ? `${IMG_W500}${movie.poster_path}` : '';
                if (!posterUrl) return;
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div style="display:flex; gap:10px; margin-top:15px;">
                    <button id="add-to-watchlist-btn" style="flex:1; padding:10px; font-size: 14px; background:var(--button-bg); color:var(--button-text); border:none; border-radius:12px; cursor:pointer; font-weight:800; display:flex; align-items:center; justify-content:center; gap:8px;"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor" style="display:flex;align-items:center;justify-content:center;"><path d="M432-93.5Q412-113 412-142v-269H142q-29 0-48.5-19.5t-19.5-48Q74-507 93.5-527t48.5-20h269v-271q0-29 19.5-48.5t48-19.5q28.5 0 48.5 19.5t20 48.5v270h271q29 0 48.5 20t19.5 48q0 28-19.5 48T818-412H548v270q0 29-20 48.5T480-74q-28 0-48-19.5Z"></path></svg> Watchlist</button>
                    <button id="add-to-favorites-btn" style="flex:1; padding:10px; font-size: 14px; background:#ff4444; color:white; border:none; border-radius:12px; cursor:pointer; font-weight:800; display:flex; align-items:center; justify-content:center; gap:8px;"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#e3e3e3" style="display:flex;align-items:center;justify-content:center;"><path d="M430.5-116.5Q407-126 387-144l-77-70q-102-93-189-193T34-637q0-113 75.5-189.5T298-903q54 0 100 19t81 48q35-29 81-48t100-19q113 0 190 76.5T927-637q0 130-88.5 230.5T647-213l-75 70q-20 18-44 27t-49 9q-25 0-48.5-9.5Z"></path></svg> Favorites</button>
                </div>
